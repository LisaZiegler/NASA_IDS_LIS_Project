...
!----------------surface forcing: nouniform meteo conditions-------------------!
   ...
   REAL(SP), ALLOCATABLE ::  NETSWN(:,:), NETLWN(:,:) ! net short and longwave radiation, at nodes
   REAL(SP), ALLOCATABLE  :: EL_AIR(:)
   REAL(SP), ALLOCATABLE  :: EGF_AIR(:)
   REAL(SP), ALLOCATABLE  :: ELF_AIR(:)   ! something crazy
   REAL(SP), ALLOCATABLE  :: ELRK_AIR(:)

   REAL(SP), ALLOCATABLE  :: QA_AIRN(:,:)        !!SPECIFIC HUMIDITY  ! for Ice mod, will fill an array in input anyways, but not used in mod_heatflux
...
REAL(SP), ALLOCATABLE :: NETSW(:),NETLW(:) ! net short and long wave radiation,
                                           ! B Clark July 2015
...
!
   IF(M_TYPE == 'uniform')THEN
     ...
     write(IPT,*)'opened the mc_air.dat file' ! B Clark debug

   ELSE 
     CALL FOPEN(INHFX_AIR, TRIM(ISTR)//'_hfx_air.dat' ,"cfr")
   
     write(IPT,*)'opened the hfx_air.dat file' ! B Clark debug
   END IF
...
!------------------------------------------------------------------------------|
   ...
   REAL(SP), ALLOCATABLE :: RTEMP6(:,:),RTEMP7(:,:),RTEMP8(:,:),RTEMP9(:,:)
...
!==============================================================================|
     ...
     IF(MSR)WRITE(IPT,*)'Forcing for uniform Heat flux'
...
!
     ...
     ALLOCATE(NETSW(NCNT)) ! Added by B Clark for net heat calc
     ALLOCATE(NETLW(NCNT))
     DO I=1,NCNT
...
!                                                                         short wave radiation (?)
       READ(INCWH_AIR ,*) T_AIRU(I), RH_AIRU(I), PA_AIRU(I), DLW_AIRU(I), DSW_AIRU(I),NETSW(I),NETLW(I)

       IF(MSR)WRITE(IOPRT,*) T_AIRU(I), RH_AIRU(I), PA_AIRU(I), &
              DLW_AIRU(I), DSW_AIRU(I)
!If (I==1) THEN
!    write(*,*)  T_AIRU(I), RH_AIRU(i), PA_AIRU(I), &     ! B Clark debug
 !        DLW_AIRU(I), DSW_AIRU(I),NETSW(I),NETLW(I)
!ENDIF  
     END DO
...
!
     ...
     IF(MSR)WRITE(IPT,*)'Forcing for non-uniform Heat flux'
     IF(MSR)THEN
...
!      for ice model
	 ...
	 ALLOCATE(RTEMP8(NNodeGL,HFX1_TM%NTIMES),RTEMP9(NNodeGL,HFX1_TM%NTIMES))
     ...
     IF(MSR)THEN
       DO J=1,HFX1_TM%NTIMES
         ...
         READ(INHFX_AIR,*) (RTEMP1(I,J),RTEMP2(I,J),RTEMP3(I,J),RTEMP4(I,J),  &
                      RTEMP5(I,J), 								&!
					 ! RTEMP6(I,J),RTEMP7(I,J),     &! 
					  RTEMP8(I,J),RTEMP9(I,J),I=1,NNodeGL)
...
!
     ...
     IF(PAR)THEN
	   ...
	   CALL MPI_BCAST(RTEMP8,HFX1_TM%NTIMES*NNodeGL,  MPI_F,0,MPI_COMM_WORLD,IERR)
	   CALL MPI_BCAST(RTEMP9,HFX1_TM%NTIMES*NNodeGL,  MPI_F,0,MPI_COMM_WORLD,IERR)
     END IF
...
!    for ice
	 ...
	 ALLOCATE(NETSWN(NNode,HFX1_TM%NTIMES),NETLWN(NNode,HFX1_TM%NTIMES))
     ...
     IF(SERIAL)THEN
	   ...
	   NETSWN(1:NNodeGL,:) = RTEMP8(1:NNodeGL,:)
	   NETLWN(1:NNodeGL,:) = RTEMP9(1:NNodeGL,:)
     END IF
     ...
     IF(PAR)THEN
       DO I=1,NNode
		 ...
		 NETSWN(I,:)  = RTEMP8(NGID(I),:)
		 NETLWN(I,:) = RTEMP9(NGID(I),:)
       !IF(I==1) THEN
...
!     DEALLOCATE(RTEMP1,RTEMP2,RTEMP3,RTEMP4,RTEMP5)
     DEALLOCATE(RTEMP1,RTEMP2,RTEMP3,RTEMP4,RTEMP5,RTEMP6,RTEMP7,RTEMP8,RTEMP9)
...
!==============================================================================|
   ...
   REAL(SP) :: WDS,T_AIRF,RH_AIRF,PA_AIRF,DLW_AIRF,DSW_AIRF,NETSWF,NETLWF
!    for ice model
...
!
  ...
  !     write(*,*)'t_airf before bulk loop = ',T_AIRF
       ...
       NETSWF = UFACT*NETSW(L1) + FACT*NETSW(L2)
       NETLWF = UFACT*NETLW(L1) + FACT*NETLW(L2)
        DO I=1,NNode
...
!     write(*,*)'sst = ',T1(I,1) 
!     write(*,*) 'Wind speed = ',WDS 
!      write(*,*) 'NODe Number is ',I
!      write(*,*)'air temp is',T_AIRF
!      write(*,*)'RH is ',RH_AIRF
!      write(*,*)'Pressure is ', PA_AIRF
      WTSURF(I) = HSB + HLB +  NETSWF + NETLWF  ! Negative is ocean is gaining heat, positive is ocean losing heat
!  IF(I ==1) THEN 
     ...
     ELSE 
       ...
       IF(WINDTYPE == 'speed')THEN
         ...
         DO I=1,NNode
		   ...
		   NETSWF    = UFACT*NETSWN(I,L1) + FACT*NETSWN(I,L2) 
		   NETLWF    = UFACT*NETLWN(I,L1) + FACT*NETLWN(I,L2) 
!        for ice model
!----------------------------------------------
!----------------------------------------------
...
!           QA_AIR(I) = UFACT*QA_AIRN(I,L1) + FACT*QA_AIRN(I,L2)
      ...
      WTSURF(I) = HSB+HLB+NETSWF+NETLWF
