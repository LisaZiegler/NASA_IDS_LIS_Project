...
!
   INTEGER              :: MHE             !!NUMBER OF HALO ELEMENTS
...
!T.W. added here from v2.3
!Below per KGlaesemann
   TYPE(MAP), ALLOCATABLE, DIMENSION(:) :: BMAP  !boundary nodes mapping
!
...
!------------------------------------------------------------------------------
   ...
   INTEGER, INTENT(IN)             :: BN_MLT(NBN)  !number of owners for boundary nodes
   INTEGER, INTENT(IN)             :: BN_LOC(NBN)  !local node number of boundary nodes
   INTEGER, INTENT(IN)             :: NT,KT,MYID,NPROCS
   TYPE(COMM), INTENT(IN)          :: CM(NPROCS)
   REAL(SP), INTENT(INOUT)           :: A(0:NT,KT)
   REAL(SP), INTENT(INOUT), OPTIONAL :: B(0:NT,KT)
...
!       ALLOCATE(SBUF(NSZE))
       ...
       DO J=1,KT
         N1 = N2+1  ; N2 = N1 + LBUF -1
         SBUF(N1:N2) = A(CM(I)%SNDP(:),J)  !put A in the sending buffer
         IF(BYES)THEN
           N1 = N2+1 ; N2 = N1 + LBUF -1
           SBUF(N1:N2) = B(CM(I)%SNDP(:),J) !put B in the sending buffer
         END IF
         IF(CYES)THEN
           N1 = N2+1 ; N2 = N1 + LBUF -1     !put C in the sending buffer
           SBUF(N1:N2) = C(CM(I)%SNDP(:),J)
         END IF
       END DO
       TSND  = TSND + 1
...
!===================================================================================|
   DO NMSG = 1,TRCV 
     ...
     DO J=1,KT
       N1 = N2+1 ; N2 = N1 + LBUF -1
       A(CM(LPROC)%RCVP(:),J) = RBUF(N1:N2) + A(CM(LPROC)%RCVP(:),J)  !accumulates A
       IF(BYES)THEN
         N1 = N2+1; N2 = N1 + LBUF -1
         B(CM(LPROC)%RCVP(:),J) = RBUF(N1:N2) + B(CM(LPROC)%RCVP(:),J)  !accumulates B
       END IF
       IF(CYES)THEN
         N1 = N2+1 ; N2 = N1 + LBUF -1
         C(CM(LPROC)%RCVP(:),J) = RBUF(N1:N2) + C(CM(LPROC)%RCVP(:),J)  !accumulates C
       END IF
     END DO
   END DO
...
!===================================================================================|
   IF(IMATCH /=1)RETURN  !accumulation is usually used for accumulating fluxes around a node

   DO J=1,KT    !do averaging if want to enforce agreement, basically set to averaging values
     DO I=1,NBN
...
!    3.) LB2   - UPPER BOUND OF 1ST ARRAY DIMENSION (USUALLY MTElem OR NTNode)         |
