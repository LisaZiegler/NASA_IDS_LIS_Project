...
!=========================================================================|
   ...
   ALLOCATE(CXOBC(0:MTElem))                       ;CXOBC         = ZERO
   ALLOCATE(CYOBC(0:MTElem))                       ;CYOBC         = ZERO
...
!       FLUXF_OBC_1 = 0.5_SP*SQRT(E1_X**2+E1_Y**2)*  &            				 !distance 
!                   (UANP(C1)*CXOBC(ADJC_OBC(I,1))+VANP(C1)*CYOBC(ADJC_OBC(I,1)))  !CXOBC CYOBC  seems to be normal vector of OB curve
!                                                                                  !FLUXF_OBC_1 is normal velocity * length ==> flux 
!       FLUXOBN2(I) = -FLUXF_OBC_1*(H(N1)+ELT(I1)+ELP(I1))  !multiply by total depth (to get total flux)
!
!     ELSE
!       N1   = I_OBC_N(I)
!       N2   = ADJN_OBC(I,1)
...
!
!       FLUXOBN2(I) = -(FLUXF_OBC_1+FLUXF_OBC_2)*(H(N1)+ELT(I1)+ELP(I1)) !still trying to calculate flux (m^3/sec) across OBC transect segment (
!                                                                        !positive going out of domain
!     END IF
!   END DO
!   END IF   
   ...
   IF (nmfcell > 0) THEN
     DO I = 1, nmfcell
        ...
        FLUXOBN2(C1) = FLUXOBN2(C1) - MFQDIS(I)*RDISMF(I,1)  !modify the tidal flux computed at the open boundary above by the specified mean flow
                                                             !MFQDIS is positive going into domain, and FLUXOBN2 is positive going out of main
...
! 2-D and 3-D adjustment
      DO I = 1, nmfcell
       II= I_MFCELL_N(I)
         UTMP   = SUM(UNT(I,1:KBM1)*DZ1(II,1:KBM1))  			!depth integrated velocity
         VTMP   = SUM(VNT(I,1:KBM1)*DZ1(II,1:KBM1))			! 
         UNT(I,1:KBM1) = UNT(I,1:KBM1) + (UANT(I) - UTMP)               !UANT is depth averaged tidal flow 
         VNT(I,1:KBM1) = VNT(I,1:KBM1) + (VANT(I) - VTMP)               !==> UNT is velocity modified by including depth averaged tidal flow
      END DO
   END IF
...
!          TMP1 = -(UNT(I,K)*cos(ANGLEMF(I))+VNT(I,K)*sin(ANGLEMF(I))*(SQRT(DX12**2+DY12**2))
          TMP1 = UNT(I,K) * DY12 - VNT(I,K) * DX12
          FLUXOBC3D_X_2(I,K) = DTMP * TMP1 * UNT(I,K)               !u momentum flux 
          FLUXOBC3D_Y_2(I,K) = DTMP * TMP1 * VNT(I,K)               !v momentum flux 
       END DO
     END DO
