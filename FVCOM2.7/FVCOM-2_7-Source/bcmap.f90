...
! End loop
! After this is done, all the children report in to master, this uses two sends
! one to tell how many, and the second to give the map
   IF(PAR) then
     ALLOCATE(TEMPB(IOBCN_GL))
     NCNT = 0
     DO I=1,IOBCN_GL                  !loop through open boundary nodes
       I1 = NLID( I_OBC_GL(I) )       !get local node ID of the boundary nodes
       IF(I1 /= 0)THEN
         NCNT = NCNT + 1
         TEMPB(NCNT) = I
       END IF
     END DO
     if (ncnt .ne. IOBCN) call pstop    !check if consistent is local number of open boundary nodes
     IF(MYID /= 1)THEN
       ISND = MYID+150                  !send to master number of open boundary nodes
       CALL MPI_SEND(IOBCN,1,MPI_INTEGER,0,ISND,MPI_COMM_WORLD,IERR)
       ISND = MYID+170                  !send to master global node IDs of the open boundary nodes 
       CALL MPI_SEND(TEMPB, IOBCN,MPI_INTEGER,0,ISND,MPI_COMM_WORLD,IERR)
     else
       ALLOCATE(BMAP(NPROCS)) ; BMAP(:)%NSIZE = 0
       BORDER = IOBCN                   !master's own number of open boundary nodes 
       BMAP(1)%NSIZE = border
       do i = 2, nprocs                 !receive open boundary information from other processors
          ISND = i+150
          IINMPI = I - 1
!below changed ISIZE to 1 per Kurt
          CALL MPI_RECV(BORDER,1,MPI_INTEGER,iinmpi,ISND,MPI_COMM_WORLD,STAT,IERR)
          bmap(i)%NSIZE = BORDER       !master to get other processors number of open boundary nodes 
       end do
	   
       do i = 1, nprocs                !master to allocate array containing all processor's open boundary 
	                                   !nodes mapping information
          allocate(bmap(i)%loc_2_gl(bmap(i)%NSIZE))
          ISND = i+170
          IINMPI = I - 1
          if (i .eq. 1) then
             bmap(i)%loc_2_gl(:) = TEMPB(:)  !set the global ID of open boundary nodes for master
          else
             CALL MPI_RECV(bmap(i)%loc_2_gl,bmap(i)%NSIZE,MPI_INTEGER,iinmpi,ISND,MPI_COMM_WORLD,STAT,IERR)
          end if                             !receive the global ID of open boundary nodes for child processors
       end do
     end if
     deallocate(tempb)
   else
...
!==============================================================================|
   ...
   CALL MPI_GATHER(IBCN(1),1,MPI_INTEGER,TEMP2,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP2
   CALL MPI_GATHER(IBCN(2),1,MPI_INTEGER,TEMP3,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP2
   CALL MPI_GATHER(IBCN(3),1,MPI_INTEGER,TEMP4,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP3
   CALL MPI_GATHER(IBCN(4),1,MPI_INTEGER,TEMP5,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP4
   CALL MPI_GATHER(IBCN(5),1,MPI_INTEGER,TEMP6,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP5
   CALL MPI_GATHER(NOBCGEO,1,MPI_INTEGER,TEMP7,1,MPI_INTEGER,0,MPI_COMM_WORLD,IERR) !gather to TEMP6
