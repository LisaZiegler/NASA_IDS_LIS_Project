...
!==============================================================================!
!     variable list:							       !
!  vx(NNode)    :: vx(i) = x-coordinate of node i (input from mesh)	               !
!  vy(NNode)    :: vy(i) = y-coordinate of node i (input from mesh)	               !
!  nv(MElem,3)  :: nv(i:1-3) = 3 node numbers of element i                         !
!  xc(MElem)    :: xc(i) = x-coordinate of element i (calculated from vx)          !
!  yc(MElem)    :: yc(i) = y-coordinate of element i (calculated from vy)          !
!  cor(MElem)   :: cor(i) = f plane coriolis at element i                          !
!                                                                              !
!  nbe(MElem,3) :: nbe(i,1->3) = element index of 1->3 neighbors of element i      !
!  isbce(MElem) :: flag if element is on the boundary, see below for values        !
!  isonb(NNode) :: flag is node is on the boundary, see below for values           !
!                                                                              !
!  ntve(NNode)  :: the number of neighboring elements of node NNode                    !
!  nbve(NNode,ntve(NNode)) :: nbve(i,1->ntve(i)) = ntve elements containing node i     !
!  nbvt(NNode,ntve(NNode)) :: nbvt(i,j) = the node number of node i in element         ! 
...
!									       |
!   NBE(MElem,3) :  NBE(I,J) = IDENTITY OF NBOR ELMNT TO TRI I ON EDGE J           |
!   IBCE(MElem)  :  DESCRIBED IN SUBROUTINE HEADING			               |	
!   ISONB(NNode):  DESCRIBED IN SUBROUTINE HEADING			               |	
!==============================================================================|
   USE ALL_VARS
...
!
!----DETERMINE NBE(i=1:MElem,j=1:3): INDEX OF 1 to 3 NEIGHBORING ELEMENTS----------!
!
   ALLOCATE(NBET(MTElem,3)) ; NBET = 0
   ALLOCATE(CELLS(NTNode,50)) ; CELLS = 0
   ALLOCATE(CELLCNT(NTNode))  ; CELLCNT = 0
   DO I=1,MTElem
...
!                                                                              !
! ntve(1:NNode):           total number of the surrounding triangles               !
!                      connected to the given node                             !                                        
! nbve(1:NNode, 1:ntve+1): the identification number of surrounding                !
!                      triangles with a common node (counted clockwise)        !
! nbvt(1:NNode,ntve(1:NNode)): the idenfication number of a given node over            !
!                      each individual surrounding triangle(counted            !
!                      clockwise)                                              !
! ntsn(1:NNode):           total number of surrounding nodes                       !
! nbsn(1:NNode, ntsn):     the identification number of surrounding nodes          !
!                      (counted clockwise)                                     !
! nbse(1:NNode,2*ntsn):    the identification number of control volume s           !
...
! 
   ALLOCATE(NBVE(NNode,MX_NBR_ELEM+1))
   ALLOCATE(NBVT(NNode,MX_NBR_ELEM+1))
!   ALLOCATE(NBSN(NNode,MX_NBR_ELEM+2))
   ALLOCATE(NBSN(NNode,MX_NBR_ELEM+3)) !!MHB
!
...
!
   ALLOCATE(NB_TMP(NNode,MX_NBR_ELEM+1))
   DO I=1,NNode
     IF(ISONB(I) == 0) THEN
       NB_TMP(1,1)=NBVE(I,1)
       NB_TMP(1,2)=NBVT(I,1)
...
!==============================================================================!
   ...
   ALLOCATE(ISET(MTElem,3),TEMP((MTElem)*3,2),TEMP2((MTElem)*3,2))
   ISET = 0
   NE = 0
   TEMP = 0 
   TEMP2 = 0
...
!==============================================================================!
   ...
   IF(NCV /= 3*(MTElem)) THEN
     PRINT*,'NCV IS NOT CORRECT, PLEASE CHECK THE SETUP'
     CALL PSTOP
   END IF
   IF(NCV_I /= 3*MElem) THEN
     PRINT*,'NCV_I IS NOT CORRECT, PLEASE CHECK THE SETUP'
      CALL PSTOP
   END IF

   DO I=1,NCV_I
     IF(NIEC(I,1) > NNode .OR. NIEC(I,2) > NNode)THEN
       write(ipt,*)'problemas',niec(i,1),niec(i,2),NNode
       CALL PSTOP
     END IF
   END DO
...
!
   ALLOCATE(EPOR(0:MTElem)) ; EPOR = 1.0_SP
   DO I=1,MElem
