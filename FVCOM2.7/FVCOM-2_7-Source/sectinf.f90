...
!------------------------------------------------------------------------------|
   ...
   REAL(SP), ALLOCATABLE,DIMENSION(:,:) :: FTEMP1,FTEMP2,FTEMP3,FTEMP4,FTEMP5
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP1(NNodeGL,1))
     ALLOCATE(FTEMP2(NNodeGL,1))
     CALL GATHER(LBOUND(VX,1),UBOUND(VX,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,VX,FTEMP1)
     CALL GATHER(LBOUND(VY,1),UBOUND(VY,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,VY,FTEMP2)
     IF(MSR)THEN
       DO I=1,NNodeGL
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP1(MElemGL,1))
     ALLOCATE(FTEMP2(MElemGL,1))
     CALL GATHER(LBOUND(XC,1),UBOUND(XC,1),MElem,MElemGL,1,MYID,NPROCS,EMAP,XC,FTEMP1)
     CALL GATHER(LBOUND(XC,1),UBOUND(XC,1),MElem,MElemGL,1,MYID,NPROCS,EMAP,YC,FTEMP2)
     IF(MSR)THEN
       DO I=1,MElemGL
         WRITE(1,*) FTEMP1(I,1)+VXMIN,FTEMP2(I,1)+VYMIN
       END DO
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(NTEMP1(MElemGL,3))
     ALLOCATE(NTEMP2(MElemGL,3))
...
!    TRANSFORM NBE AND NV ARRAYS TO GLOBAL INDEXING
     DO J=1,3
     DO I=1,MElem
       IF(NBE(I,J) > MElem) THEN
         NBETEMP(I,J) = HE_LST(NBE(I,J)-MElem)
       ELSE
     ...
     CALL IGATHER(LBOUND(NBE,1),UBOUND(NBE,1),MElem,MElemGL,3,MYID,NPROCS,EMAP,NBETEMP,NTEMP1)
     CALL IGATHER(LBOUND(NV ,1),UBOUND(NV ,1),MElem,MElemGL,3,MYID,NPROCS,EMAP,NVTEMP ,NTEMP2)
     IF(MSR)THEN
       DO I=1,MElemGL
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP1(MElemGL,4))
     ALLOCATE(FTEMP2(MElemGL,4))
     CALL GATHER(LBOUND(A1U,1),UBOUND(A1U,1),MElem,MElemGL,4,MYID,NPROCS,EMAP,A1U,FTEMP1)
     CALL GATHER(LBOUND(A2U,1),UBOUND(A2U,1),MElem,MElemGL,4,MYID,NPROCS,EMAP,A2U,FTEMP2)
     IF(MSR)THEN
       DO I=1,MElemGL
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP1(MElemGL,3))
     ALLOCATE(FTEMP2(MElemGL,3))
     ALLOCATE(FTEMP3(MElemGL,3))
     CALL GATHER(LBOUND(AW0,1),UBOUND(AW0,1),MElem,MElemGL,3,MYID,NPROCS,EMAP,AW0,FTEMP1)
     CALL GATHER(LBOUND(AWX,1),UBOUND(AWX,1),MElem,MElemGL,3,MYID,NPROCS,EMAP,AWX,FTEMP2)
     CALL GATHER(LBOUND(AWY,1),UBOUND(AWY,1),MElem,MElemGL,3,MYID,NPROCS,EMAP,AWY,FTEMP3)
     IF(MSR)THEN
       DO I=1,MElemGL
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP1(NNodeGL,1))
     ALLOCATE(FTEMP2(NNodeGL,1))
     ALLOCATE(FTEMP3(NNodeGL,1))
     CALL GATHER(LBOUND(VX,1),UBOUND(VX,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,VX,FTEMP1)
     CALL GATHER(LBOUND(VY,1),UBOUND(VY,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,VY,FTEMP2)
     CALL GATHER(LBOUND(H ,1),UBOUND(H ,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,H ,FTEMP3)
     IF(MSR)THEN
       DO I=1,NNodeGL
...
!
   ...
   IF(PAR)THEN
     ALLOCATE(FTEMP4(NNodeGL,KB))
     CALL GATHER(LBOUND(Z ,1),UBOUND(Z ,1),NNode,NNodeGL,KB,MYID,NPROCS,NMAP,Z ,FTEMP4)
     IF(MSR)THEN
       DO I=1,NNodeGL
...
!ADDED BY Z. YANG, 2/22/2010
!
!----------OUTPUT THE INFORMATION OF THE FIRST NEIGHBOR CELL OF A NODE --------|
!
   IF(MSR)OPEN(1,FILE=TRIM(TEMPDIR)//'bin2rma2.inf',STATUS='unknown') ; REWIND(1)

   IF(SERIAL)THEN
     DO I=1,NNode
!	 NELM = NTVE(I)
       WRITE(1,*) I, NTVE(I),(NBVE(I,J),J=1,NTVE(I))
     END DO
   END IF

!#  if defined (1)
!   IF(PAR)THEN
!     ALLOCATE(FTEMP5(NNodeGL,6))
!     CALL GATHER(LBOUND(NBVE,1),UBOUND(NBVE,1),NNode,NNodeGL,1,MYID,NPROCS,NMAP,NBVE,FTEMP5)
!     IF(MSR)THEN
!       DO I=1,NNodeGL
!         WRITE(1,*) I,FTEMP5(I,1)
!       END DO
!     END IF
!     DEALLOCATE(FTEMP5)
!   END IF
