...
TEMP_in  = nc{'temp'}(:);
KD_in = nc{'KD'}(:);
CHLA_in = (alg1_in./50+alg2_in./50).*1000;
...
for istat=1:length(stations_mod);
 ...
 %This contours the model output for this station   
    ...
    hold on
    [cmin,cmax]=caxis;
    xlabel('Days in 2018');ylabel('Depth (m)')
    ...
    xlabel('Days in 2018');ylabel('Chl a (ug l^-^1)');
    ...
    xlabel('Days in 2018');ylabel('Observed DO (mg O_2 l^-^1)');
    legend('Modeled','Observed');
    subplot(2,2,3);
    plot(X_out_mean,Y_out_mean,'rd','markersize',10);
    axis([axis_min axis_max axis_min axis_max]);
   ...
   %{ 
    ...
    while isempty(myvar_id);
        ivar=ivar+1;
        if(strcmp(stations(istat).variables(ivar).name,'NO3'))
            myvar_id=ivar              ; % change this ID based on where the variable is located within the CBP data structure
            % generated by read_cbp.m
        end
    end
   ...
   %}
    ...
    while isempty(myvar_id);
        ivar=ivar+1;
        if(strcmp(stations(istat).variables(ivar).name,'NH4'));
            myvar_id=ivar              ; % change this ID based on where the variable is located within the CBP data structure
            % generated by read_cbp.m
        end
    end
    ...
    xlabel('Days in 2018');ylabel('NH_4^+ (mg N l^-^1)')  ;
    legend('Modeled','Observed')
    subplot(2,2,3);
    plot(X_out_mean,Y_out_mean,'rd','markersize',10);
    axis([axis_min axis_max axis_min axis_max]);
    ...
    xlabel('Days in 2018');ylabel('DON(mg N l^-^1)')
    legend('Modeled','Observed')
    subplot(2,2,3);
    plot(X_out_mean,Y_out_mean,'rd','markersize',10);
    axis([axis_min axis_max axis_min axis_max]);
    ...
    xlabel('Days in 2018');ylabel('DOC(mg C l^-^1)');
    legend('Modeled','Observed');
    subplot(2,2,3);
    plot(X_out_mean,Y_out_mean,'rd','markersize',10);
    axis([axis_min axis_max axis_min axis_max]);
   ...
   %{
    ...
    while isempty(myvar_id);
        ivar=ivar+1;
        if(strcmp(stations(istat).variables(ivar).name,'Temperature'));
    ...
    xlabel('Days in 2018');ylabel('Temperature')
...
%% Make monthly surface plots
t1 = datetime(2018,01,01,00,00,00,'format','yyyy/MM/dd');
t2 = datetime(2018,12,31,00,00,00,'format','yyyy/MM/dd');
t_stat = (t1:days:t2)';
dn = datenum(t_stat);
[y,m,d] = datevec(dn);
...
% finding the daily outputs

CHLA_d = squeeze(CHLA_in(:,1,:));
DO_d = squeeze(DO_in(:,1,:));
NH4_d = squeeze(NH4_in(:,1,:));
NO3_d = squeeze(NO3_in(:,1,:));
DON_d = squeeze(DON_in(:,1,:));
DOC_d = squeeze(DOC_in(:,1,:));
POC_d = squeeze(POC_in(:,1,:));
PON_d = squeeze(PON_in(:,1,:));
SALT_d = squeeze(SALT_in(:,1,:));
TEMP_d = squeeze(TEMP_in(:,1,:));

big_chla = zeros(365,7478);
big_do = zeros(365,7478);
big_nh4 = zeros(365,7478);
big_no3 = zeros(365,7478);
big_don = zeros(365,7478);
big_doc = zeros(365,7478);
big_poc = zeros(365,7478);
big_pon = zeros(365,7478);
big_salt = zeros(365,7478);
big_temp = zeros(365,7478);

for i = 1:365;
    
if i*24<8758;
    
big_chla(i,:)=mean(CHLA_d((i-1)*24+1:i*24,:));
big_do(i,:)=mean(DO_d((i-1)*24+1:i*24,:));
big_nh4(i,:)=mean(NH4_d((i-1)*24+1:i*24,:));
big_no3(i,:)=mean(NO3_d((i-1)*24+1:i*24,:));
big_don(i,:)=mean(DON_d((i-1)*24+1:i*24,:));
big_doc(i,:)=mean(DOC_d((i-1)*24+1:i*24,:));
big_poc(i,:)=mean(POC_d((i-1)*24+1:i*24,:));
big_pon(i,:)=mean(PON_d((i-1)*24+1:i*24,:));
big_salt(i,:)=mean(SALT_d((i-1)*24+1:i*24,:));
big_temp(i,:)=mean(TEMP_d((i-1)*24+1:i*24,:));

else;
    
big_chla(i,:)=mean(CHLA_d((i-1)*24+1:end,:));
big_do(i,:)=mean(DO_d((i-1)*24+1:end,:));
big_nh4(i,:)=mean(NH4_d((i-1)*24+1:end,:));
big_no3(i,:)=mean(NO3_d((i-1)*24+1:end,:));
big_don(i,:)=mean(DON_d((i-1)*24+1:end,:));
big_doc(i,:)=mean(DOC_d((i-1)*24+1:end,:));
big_poc(i,:)=mean(POC_d((i-1)*24+1:end,:));
big_pon(i,:)=mean(PON_d((i-1)*24+1:end,:));
big_salt(i,:)=mean(SALT_d((i-1)*24+1:end,:));
big_temp(i,:)=mean(TEMP_d((i-1)*24+1:end,:));
...
k=k+1;
idx=find(y==j&m==i);
big_chla_mon(k,:)=mean(big_chla(idx,:));
big_do_mon(k,:)=mean(big_do(idx,:));
big_nh4_mon(k,:)=mean(big_nh4(idx,:));
big_no3_mon(k,:)=mean(big_no3(idx,:));
big_don_mon(k,:)=mean(big_don(idx,:));
big_doc_mon(k,:)=mean(big_doc(idx,:));
big_poc_mon(k,:)=mean(big_poc(idx,:));
big_pon_mon(k,:)=mean(big_pon(idx,:));
big_salt_mon(k,:)=mean(big_salt(idx,:));
big_temp_mon(k,:)=mean(big_temp(idx,:));
end
end
clear *_in;

save('bgc_variables', 'big_chla','big_do','big_nh4','big_no3','big_don','big_doc','big_poc','big_pon', ...
    'big_chla_mon','big_do_mon','big_nh4_mon','big_no3_mon','big_don_mon','big_doc_mon','big_poc_mon','big_pon_mon');