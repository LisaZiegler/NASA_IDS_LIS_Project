...
EE61_variables = n7477_variables.variables;

% get the dates for this location
unique_dates = unique(EE61_variables(3).date); % variables(1);
...
DON = mean_EE61_variables(4).values;
CDON=DON*CDOMFrac;
...
DOP=mean_EE61_variables(5).values;
CDOP=DOP*CDOMFrac;
...
% nitrogen and DON
PON = mean_EE61_variables(10).values-DON;
...
myRiver_array = zeros(44,1,20); % (44,4,12);

my_dates = unique_dates*24;

%%
%%%%temperature
myRiver_array(1,:,:)=repmat(mean_EE61_variables(13).values,1,1);
%Salinity
2
%ssi
3
...
%%%%NH4
myRiver_array(13,:,:)=repmat(mean_EE61_variables(6).values,1,1);
%%%%NO3
myRiver_array(14,:,:)=repmat(mean_EE61_variables(7).values,1,1);
% Urea
15
%%%%CDON1
...
%%%%PO4
myRiver_array(20,:,:)=repmat(mean_EE61_variables(8).values,1,1);
%%%%CDOP1
...
26
%%%%DOXG
myRiver_array(27,:,:)=repmat(mean_EE61_variables(3).values,1,1)
%SIUPB
28
%SIAT
29
...
fid=fopen('Blackwater_river_waterquality.dat','w');
