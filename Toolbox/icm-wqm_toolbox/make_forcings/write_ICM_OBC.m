...
%data strucutre with observationaal data organized using ReadCBP
structin='/Users/lisaziegler/Desktop/Water_Quality_forcing/Tonic_wq_input/OBC/tonic_obc_stationH4.mat';
...
gridfilein='/Users/lisaziegler/Desktop/Water_Quality_forcing/Tonic_wq_input/Tonic_26Jul.mat';
%grid geometry file from sm2sdm2mat
load(gridfilein);

temp_obc_file=importdata('/Users/lisaziegler/Desktop/Water_Quality_forcing/Tonic_wq_input/OBC/tonic_obc.dat')
%obc boundary file with the obc node numbers
boundary_ids = temp_obc_file.data(:,2);
...
% clear nc
first_day=input('What isthe first day of the model? e.g. April 1 is 91 --->  ');
last_day=input('What is the last day of the model? e.g. Nov 30 is 334 ---->  ');
     ...
     date_counter=[];
         for jtime=1:length(unique_dates); % now go into each date and interpolate to the model node depth
             ...
             if(length(unique_sorted_depths)>1);  % interpolate in depth range      
              ...
              time_array_out=first_day:14:last_day; % biweekly time series
...
if(write_flag)
    ...
    carbon2chl1=50; %carbon to chlorophyll a ratio micro g C/ micro chl
    carbon2chl2=50;
    ...
    alg1_frac=0.5; %fraction of algal biomass that is diatoms;
    ...
    TDP_id=find(strcmp(var_names_out,'TDP'));
    TND_id=find(strcmp(var_names_out,'TDN'));
    PP_id=find(strcmp(var_names_out,'PP'));
    PN_id=find(strcmp(var_names_out,'PN'));
    ...
    TON = ones(size(DON));
    TON(:,:,:) = 0.16;
    PON = TON - squeeze(time_interped_out(:,:,DON_id,:)); % mg N l^-1
    ...
    %%
    obc_file_out='TonicFVM_OBC_Nov2019.dat'; %printing file
