...
%
%fname=input('What string would you like to name your file with? --->','s');
wqm_hisdir = '/Volumes/LaCie/Housatonic_BGC/Inputs and data';
nc1 = [wqm_hisdir '/tonic_0364.nc'];
nvars = 44;
%nc1=input('What file would you like to use for the new initial condition? -->','s');

nc=netcdf(nc1);
Big_array_out(:,:,1)=nc{'temp'}(end,:,:);%T
Big_array_out(:,:,2)=nc{'salinity'}(end,:,:);%SALT
Big_array_out(:,:,3)=10.0; nc{'SSI'}(end,:,:);
Big_array_out(:,:,4)=nc{'B1'}(end,:,:); %ALG1 0.05
Big_array_out(:,:,5)=nc{'B2'}(end,:,:); %ALG2 0.05
...
Big_array_out(:,:,13)=0.001;%nc{'NH4'}(end,:,:); %NH4
Big_array_out(:,:,14)=0.13;%nc{'NO3'}(end,:,:);  %N03
...
Big_array_out(:,:,27)=12.0;%nc{'DOXG'}(end,:,:); %DO2
...
%     for jlay=1:nlayers;
%         for kvars = 1:nvars
%             fprintf(fid,'%5e',Big_array_out(jlay,igrid,kvars));
%         end
%         fprintf(fid,'\n');
%     end
% end
fid = fopen('tonic_initial_wq_vert_restart1_updated10Aug21.dat','w');
